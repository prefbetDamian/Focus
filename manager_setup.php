<?php
/**
 * ⚠️ TEN PLIK ZOSTAŁ PRZENIESIONY
 * 
 * Panel administratora został przeniesiony do zabezpieczonego katalogu.
 * Dostęp: http://localhost/Rcp/admin/
 * 
 * Dokumentacja: ADMIN_SETUP.md
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        if (!$firstName || !$lastName) {
            ...
            try {
                $token = bin2hex(random_bytes(16)); // 32-znakowy token
                $stmt = $pdo->prepare("
                    INSERT INTO managers (first_name, last_name, email, phone, pin_token, role_level)
                    VALUES (?, ?, ?, ?, ?, ?)
                ");
                $stmt->execute([$firstName, $lastName, $email, $phone, $token, $role]);
                ...
                if (!empty($email) && filter_var($email, FILTER_VALIDATE_EMAIL)) {
                    ...
                    $body .= "<p>Utworzono dla Ciebie konto kierownika w systemie RCP.</p>";
        ...
        if (!$id) {
            ...
            try {
                $token = bin2hex(random_bytes(16)); // Nowy token
                $stmt = $pdo->prepare("
                    UPDATE managers
...
$stmt = $pdo->query("
    SELECT id, first_name, last_name, email, phone, role_level, device_id, ip_address, created_at, pin_token, pin_hash
    FROM managers
    ORDER BY id
");
...
<body>
    <div class="container">
            ...
            <form method="POST">
                ...
                <div class="form-group">
                    ...
                    <select name="role" required>
                        ...
                        <option value="5">Starszy kierownik (5)</option>
        ...
        <div style="text-align: center; margin-top: 30px;">
            <a href="index.html" style="color: #667eea; text-decoration: none; font-weight: bold;">← Powrót do strony głównej</a>
        </div>
    </div>
</body>
</html>
